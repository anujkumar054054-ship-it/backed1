...
// Tracks users who clicked the YouTube subscribe link
...
app.all("/api/bot/refer", async (req, res) => {
  try {
    ...
    if (ref && user.referred_by === (ref || null)) {
      // Reward is now handled separately via /api/bot/refer-reward
      // which is called by the webapp AFTER channel verification passes.
      // This keeps device_id synced (webapp has it) and ensures reward
      // is only given to users who actually complete onboarding.
    }
...
// 7b. WEBAPP REFERRAL REWARD — called only after channel verification passes
// Registration already happened via /api/bot/refer at boot.
// This endpoint is idempotent — calling it twice does nothing.
app.all("/api/bot/refer-reward", async (req, res) => {
  try {
    ...
    // Block reward for duplicate devices
    ...
    // Idempotency — only reward once per referred user
    ...
    console.error("Refer-reward error:", err);
...
// YouTube: Mark user as having visited the subscribe link
